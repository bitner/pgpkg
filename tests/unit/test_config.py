...
def test_load_config_sample(sample_project: Path):
    ...
    assert c.version_source is None
...
def test_load_config_with_version_source(tmp_path: Path):
    (tmp_path / "pyproject.toml").write_text(
        "\n".join(
            [
                "[tool.pgpkg]",
                'project_name = "custom"',
                'version_source = "custom.module:Source"',
            ]
        )
    )
    c = load_config(tmp_path)
    assert c.project_name == "custom"
    assert c.version_source == "custom.module:Source"


def test_load_config_rejects_pre_post_in_base(tmp_path: Path):
    (tmp_path / "pyproject.toml").write_text(
        "\n".join(
            [
                "[tool.pgpkg]",
    ...
    with pytest.raises(ConfigError, match="pre_post_in_base"):
        load_config(tmp_path)
